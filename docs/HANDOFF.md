...
必须准确理解当前完成度：DFR1221 的 ST77916 实体圆屏、网络、状态协议、PSRAM 和麦克风初始化已在真机验证。屏幕已稳定显示蓝底、状态圆环和英文文字，串口仍保留为诊断镜像。当前固件没有包含覆盖任意中文的字库，因此中文消息会缺字；麦克风只完成了 PDM 初始化，尚无 WakeNet、录音、VAD、音频上传或 STT；CST816S 已初始化并注册为 LVGL 输入设备，但尚未接入 Orb 确认/拒绝等业务交互。
...
2. **接入触摸交互**：CST816S 已按 SCL=8、SDA=7、INT=41、RST=40 初始化，把触摸事件映射成屏幕确认/拒绝动作并接到 `GatewayClient`。
3. **补齐中文字库**：根据实际 UI 文案生成 LVGL 字体子集，或引入可覆盖动态回答的中文字库；注意内部 RAM 和 Flash 占用。当前不要宣称任意中文已可显示。
4. **对接 Snoopy 记忆例外审核**：当前 `SnoopyAssistant` 只在标题提示待审核候选数量，Orb 的确认动作还没有调用 Snoopy candidate accept/reject API。
5. **使用长轮询**：固件目前每 800ms 请求 `/state`；Gateway 已有 `/events?after=N&timeout=20`。
6. **持久运行 Gateway**：当前由终端手动启动，后续可增加独立 LaunchAgent，并继续从 Keychain 注入 Token。
7. **加强 Gateway 安全**：如果跨可信单机/局域网使用，增加认证、来源限制和更窄的 CORS。
