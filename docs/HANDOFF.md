...
- 去偏置后必须重新校准 VAD 阈值，不能沿用原来的 500。真人“现在几点”现场峰值为 216–306、底噪为 21–30；当前最小阈值为 80，并以三倍动态底噪上调。开始录音的前 400ms 不参与语音判定，避免把 `Hi ESP` 尾音误当成问题。最终真人测试在 2459ms 检测到说完并成功进入 STT/Agent 链路。
...
- 生产环境仍把一般问题交给 Snoopy Agent；由于 Snoopy 本身对“现在几点”回答“不知道”，Gateway 为时间/日期提供确定性的本机 `clock` 工具。2026-08-20 实测设备查询返回 `tool=clock` 和本机时间，其他问题不被该路由截获。
